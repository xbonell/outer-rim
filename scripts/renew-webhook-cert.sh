...
if [ -f "$SCRIPT_DIR/../nginx/certs/webhook.xbonell.com.crt" ]; then
    # Check certificate expiration (renew if less than 30 days)
    EXPIRY_DATE=$(openssl x509 -enddate -noout -in "$SCRIPT_DIR/../nginx/certs/webhook.xbonell.com.crt" | cut -d= -f2)
    ...
    if [ $DAYS_UNTIL_EXPIRY -lt 30 ]; then
        ...
        echo "$(date): Certificate is still valid, no renewal needed" | tee -a "$LOG_FILE"
    fi
else
